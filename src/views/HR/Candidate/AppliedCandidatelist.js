...
const AppliedCandidate = (props) => {
  ...
  const fetchAllLists = (searchQuery) => {
    storeObjectData(config.AllCandidate, false);

    if (roles.includes("HR")) {
      try {
        setLoading(true);
        var url = `vacancy/${id}/applied-candidates`;
        storeObjectData(config.SelectedVacancy, id);
        props
          .ongetappliedcandidate(url)
          .then((response) => {
            setLoading(false);
            // debugger;
                ...
                row["actions"] = (
                  <div className="action-btn">
                    <Tooltip title="Candidate Details">
                      <Link
                        to={"/appliedcandidatedetail/" + items[i].candidateId}
                        data-toggle="tooltip"
  ...
  return (
    <>
      <div className="form-header">
        <h5 className="lineformtitle">Applied Candidates</h5>
      </div>
